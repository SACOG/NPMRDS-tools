...
class BCP():
    ...
    def add_quotes_to_tstamp(self, in_file, tstamp_col):
        ...
        print(f"\tquoting timestamp col {tstamp_col} so it can be read into SQL Server...")
        import csv
    ...
    def create_sql_table_from_file(self, file_in, str_create_table_sql, tbl_name,
                                   overwrite=True, data_start_row=2, delimiter=None, dt_col=None,
                                   str_load2final_sql=None):
        ...
        PARAMETERS:
            ...
            dt_col = optional argument specifying field that has a date or timestamp. BCP cannot directly
                load timestamp data to SQL Server, so need to let it know if there is such a field.
            str_load2final_sql = optional argument for query that, after data have loaded into a staging table
        ...
        if dt_col and not str_load2final_sql:
            raise Exception("You specified a datetime column (dt_col). If loading a timestamp column, " \
        ...
        if dt_col:
            in_file_dt_str = self.add_quotes_to_tstamp(file_in, dt_col)
...
if __name__ == '__main__':
    ...
    #----------------TEST PARAMS---------------------
    test_file_in = r"P:/NPMRDS data/Raw Downloads/DynamicData_15Min/2020/NPMRDS2020_Trucks/NPMRDS2020_Trucks.csv"  # r"P:\NPMRDS data\Raw Downloads\DynamicData_15Min\2020\test_trucks_Dec1_2020_yuba\test_trucks_Dec1_2020_yuba.csv"  # r"P:\NPMRDS data\Python\Data Prep\LoadRawData\samplett.csv"
    sql_to_run = r"P:\NPMRDS data\Python\Data Prep\LoadRawData\qry\create_tt_table_2ph.sql"  # r"P:\NPMRDS data\Python\Data Prep\LoadRawData\qry\create_tt_table_no_tstamp.sql"  # r"P:\NPMRDS data\Python\Data Prep\LoadRawData\qry\create_tt_table.sql" # create_tt_table_chartime
    data_first_row = 2
    tstamp_col = 'measurement_tstamp'
    test_tbl_name = "TEST_BCP_trucktbl"
    load2final_sql = r"P:\NPMRDS data\Python\Data Prep\LoadRawData\qry\tt_tbl_load2final.sql"
    ...
    tbl_loader.create_sql_table_from_file(test_file_in, raw_sql_maketbls, test_tbl_name,
                                          ...
                                          dt_col=tstamp_col, str_load2final_sql=raw_sql_load2final)
