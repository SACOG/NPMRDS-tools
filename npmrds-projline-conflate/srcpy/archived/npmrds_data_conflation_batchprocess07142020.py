...
'''
#--------------------------------
# Name:npmrds_data_conflation_cmp_combdf.py
...
class NPMRDS:
    def __init__(self, fc_speed_data):
        ...
        self.col_truckpct = "Trk_Veh_Pc"
        self.truck_data_calc_dict = {self.col_truckpct: self.calc_distwt_avg}
    ...
    def make_speed_chart(self, master_df, proj_id, imgdir):
        '''DEPENDS ON HAVING MULTIPLE YEARS OF DATA'''
        ...
        dict_rename = {self.fld_value: 'Speed MPH'}
        ...
        fig = px.bar(dfc, x=self.fld_dir_out, y=dict_rename[self.fld_value], color=fld_year_str, 
                     barmode='group', title=chart_title, height=400)
...
if __name__ == '__main__':
    ...
    corridors_fc = r'I:\Projects\Darren\CMPs\CMP2017\GDB\CMP Roads.gdb\Deficient_Corridors'  # 'Deficient_Corridors_testsample'  # r'I:\Projects\Darren\CMPs\CMP2017\GDB\CMP Roads.gdb\Deficient_Corridors'
    ...
    for pid in proj_ids:
        output_obj.make_speed_chart(output_df, pid, out_img_dir)
        output_obj.make_lottr_charts(output_df, pid, out_img_dir)
...
'''
fig = px.bar(df_speed1, x='direction', y='value', color='str_datayear', barmode='group')
plot(fig)
